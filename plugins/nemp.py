...
async def _poll_single_mod(the_poller, mod_name):
    """Poll a single mod, returning (poll_results, failed)."""
    statuses, exception = await the_poller.check_mod(mod_name)
    if exception:
        return ([], [FailedModEntry(name=mod_name, exception=exception)])
    return ([(mod_name, statuses)], [])


async def announcement_drain_task(handle, _pipe):
    """Drains the NEMP announcement queue at a safe IRC rate."""
    router = handle.base["router"]
    plugin = handle.base["plugin"]
    queue = plugin.announcement_queue
...
async def polling_task(handle, _pipe):
    plugin = handle.base["plugin"]
    the_poller = plugin.poller
    announcement_queue = plugin.announcement_queue
    channels = handle.base["channels"]
    router = handle.base["router"]
    sleep_time = handle.base["poll_time"]
    ...
    while True:
        try:
            ...
            plugin.poll_cycle_count += 1

            if staff_channel and plugin.poll_cycle_count % 10 == 0:
                await router.send_message(staff_channel, f"Full cycles completed: {plugin.poll_cycle_count}")
                if plugin.auto_disabled_mods:
                    await router.send_message(
                        staff_channel,
                        f"There are {len(plugin.auto_disabled_mods)} failed mod(s)",
                    )

            # --- Failed mods tracking ---
            current_troubled_mods = list(plugin.troubled_mods.keys())
            ...
            for item in failed:  # type: FailedModEntry
                ...
                if isinstance(exception, (poller.NEMPException,)):
                    ...
                    if mod_name in plugin.troubled_mods:
                    ...
                    if mod_name not in plugin.troubled_mods:
            ...
            for mod_name in current_troubled_mods:
                nemp_logger.debug(
                    f"Mod {mod_name} is working again. Counter reset (Counter was at {plugin.troubled_mods[mod_name]}) "
                )
                del plugin.troubled_mods[mod_name]
...
class Plugin:
    ...
    async def setup(self, router, startup):
        if startup:
            ...
            if self._is_running(router):
                self._stop_polling(router)
        ...
        if startup:
            ...
            if polling_config.get("auto_start") and polling_config.get("channel"):
                ...
                await self._start_polling(router, interval, channel)

    async def teardown(self, router):
        if self.poller and hasattr(self.poller, "session") and self.poller.session:
            await self.poller.session.close()

    def _is_running(self, router):
        exists, running = router.task_pool.check_status(TASK_NAME)
        return exists and running

    async def _start_polling(self, router, timer, channel):
        ...
        router.task_pool.add_task(
            ...
            {"router": router, "plugin": self},
        )

    def _stop_polling(self, router):
        router.task_pool.cancel_task(TASK_NAME)
    ...
    async def cmd_enable(self, router, name, params, channel, userdata, rank):
        if self._is_running(router):
        ...
        await self._start_polling(router, timer_for_polls, channel)

    @subcommand("nemp", "disable", permission=Permission.OP)
    async def cmd_disable(self, router, name, params, channel, userdata, rank):
        if not self._is_running(router):
    ...
    @subcommand("nemp", "about")
    async def cmd_about(self, router, name, params, channel, userdata, rank):
    ...
    @subcommand("nemp", "help")
    async def cmd_help(self, router, name, params, channel, userdata, rank):
        if len(params) == 1:
            await router.send_message(
                channel,
    ...
    @subcommand("nemp", "status")
    async def cmd_status(self, router, name, params, channel, userdata, rank):
        if self._is_running(router):
    ...
    @subcommand("nemp", "poll", permission=Permission.OP)
    async def cmd_poll(self, router, name, params, channel, userdata, rank):
        if len(params) < 3:
    ...
    @subcommand("nemp", "list", permission=Permission.OP)
    async def cmd_list(self, router, name, params, channel, userdata, rank):
        dest = None
        if len(params) > 1:
    ...
    @subcommand("nemp", "reload", permission=Permission.OP)
    async def cmd_reload(self, router, name, params, channel, userdata, rank):
        if self._is_running(router):
            self._stop_polling(router)
    ...
    @subcommand("nemp", "test", permission=Permission.OP)
    async def cmd_test(self, router, name, params, channel, userdata, rank):
        if len(params) != 2:
            await router.send_message(
                channel,
    ...
    @subcommand("nemp", "html", permission=Permission.OP)
    async def cmd_html(self, router, name, params, channel, userdata, rank):
        self.poller.build_html()
        await router.send_message(channel, name + ": Done.")

    @subcommand("nemp", "set", permission=Permission.OP)
    async def cmd_set(self, router, name, params, channel, userdata, rank):
        if len(params) < 4:
    ...
    @subcommand("nemp", "showinfo")
    async def cmd_show_info(self, router, name, params, channel, userdata, rank):
        if len(params) < 2:
    ...
    @subcommand("nemp", "url")
    async def cmd_url(self, router, name, params, channel, userdata, rank):
        if len(params) < 2:
            await router.send_message(channel, name + ": You have to specify at least the mod's name.")
            return
    ...
    @subcommand("nemp", "disabledmods", permission=Permission.OP)
    async def cmd_disabled_mods(self, router, name, params, channel, userdata, rank):
    ...
    @subcommand("nemp", "failedmods", permission=Permission.OP)
    async def cmd_failed_mods(self, router, name, params, channel, userdata, rank):
    ...
    @subcommand("nemp", "resetfailed", permission=Permission.OP)
    async def cmd_reset_failed(self, router, name, params, channel, userdata, rank):
    ...
    @subcommand("nemp", "failcount")
    async def cmd_fail_count(self, router, name, params, channel, userdata, rank):
    ...
    @subcommand("nemp", "reloadblocklist", permission=Permission.OP)
    async def cmd_reload_blocklist(self, router, name, params, channel, userdata, rank):
    ...
    # -- ALIASES -- #
    @subcommand("nemp", "start", permission=Permission.OP)
    async def alias_start(self, router, name, params, channel, userdata, rank):
        await self.cmd_enable(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "stop", permission=Permission.OP)
    async def alias_stop(self, router, name, params, channel, userdata, rank):
        await self.cmd_disable(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "running")
    async def alias_running(self, router, name, params, channel, userdata, rank):
        await self.cmd_status(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "polling", permission=Permission.OP)
    async def alias_polling(self, router, name, params, channel, userdata, rank):
        await self.cmd_status(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "refresh", permission=Permission.OP)
    async def alias_refresh(self, router, name, params, channel, userdata, rank):
        await self.cmd_reload(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "disabled", permission=Permission.OP)
    async def alias_disabled(self, router, name, params, channel, userdata, rank):
        await self.cmd_disabled_mods(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "failed", permission=Permission.OP)
    async def alias_failed(self, router, name, params, channel, userdata, rank):
        await self.cmd_failed_mods(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "cleanfailed", permission=Permission.OP)
    async def alias_cleanfailed(self, router, name, params, channel, userdata, rank):
        await self.cmd_reset_failed(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "show", permission=Permission.OP)
    async def alias_show(self, router, name, params, channel, userdata, rank):
        await self.cmd_show_info(router, name, params, channel, userdata, rank)

    @subcommand("nemp", "reloadblocklists", permission=Permission.OP)
    async def alias_reloadblocklists(self, router, name, params, channel, userdata, rank):
        await self.cmd_reload_blocklist(router, name, params, channel, userdata, rank)
