...
async def _reload(router, name, params, channel, userdata, rank, is_channel):
    ...
    # Teardown the old plugin before reloading
    if plugin.teardown:
        await plugin.teardown(router)
    ...
    if plugin_cls is not None:
        ...
        instance = plugin_cls()
        router.add_plugin(module.PLUGIN_ID, instance, module, path)
    else:
        # Old-style plugin
    ...
    if len(new_files) == 0:
        ...
        for filename in new_files:
            ...
            if plugin_cls is not None:
                # New-style plugin
                instance = plugin_cls()
                router.add_plugin(plugin_id, instance, module, filepath)
            else:
                # Old-style plugin
                setup_fn = getattr(module, "setup", None)
                if setup_fn is not None and not callable(setup_fn):
            ...
            new_plugin = router.plugins[plugin_id]
            if new_plugin.setup:
                await new_plugin.setup(router, True)
